...
client.on('ready', () => {
    ...
    const channel = client.channels.cache.get(process.env.SPAM_CHANNEL_ID)
...
client.on('messageCreate', async message => {
    if (message.content === "$captcha_completed" && message.author.id === process.env.OWNER_ID) {
        isSleeping = false;
        message.channel.send("Autocatcher Started!")
    }

    if (message.content === "$help" && message.author.id === process.env.OWNER_ID) {
    ...
    if (!isSleeping) {
        ...
        if (message.content.includes("Please tell us") && message.author.id === "716390085896962058") {
        ...
        } else if (message.content.startsWith("$say") && message.author.id == process.env.OWNER_ID) {
            let say = message.content.split(" ").slice(1).join(" ")
            message.channel.send(say)

        } else if (message.content.startsWith("$react") && message.author.id == process.env.OWNER_ID) {
            let msg
            try {
        ...
        } else if (message.content.startsWith("$click") && message.author.id == process.env.OWNER_ID) {
            ...
            if (message?.embeds[0]?.footer?.text.includes("Spawns Remaining")) {
                ...
                const channel6 = client.channels.cache.get(process.env.LOG_CHANNEL_ID)
